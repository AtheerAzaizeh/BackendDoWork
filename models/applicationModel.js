...
const Application = {
  create: (application, callback) => {
    const query =
      "INSERT INTO tbl_108_dowork_Applications (job_id, job_seeker_id, cover_letter, resume) VALUES (?, ?, ?, ?)";
  ...
  findById: (id, callback) => {
    const query =
      "SELECT * FROM tbl_108_dowork_Applications WHERE application_id = ?";
  ...
  findByJobSeekerId: (job_seeker_id, callback) => {
    const query =
      "SELECT * FROM tbl_108_dowork_Applications WHERE job_seeker_id = ?";
  ...
  update: (id, application, callback) => {
    const query =
      "UPDATE tbl_108_dowork_Applications SET cover_letter = ?, resume = ? WHERE application_id = ?";
    console.log("Executing query:", query, application, id);
    db.query(
      query,
      [application.cover_letter, application.resume, id],
      (err, result) => {
        if (err) {
          console.error("Error executing query:", err);
          callback(err, null);
  ...
  delete: (id, callback) => {
    const query =
      "DELETE FROM tbl_108_dowork_Applications WHERE application_id = ?";
    console.log("Executing query:", query, id);
    db.query(query, [id], (err, result) => {
      if (err) {
        console.error("Error executing query:", err);
